...
export const uploadLogo = async (token: string, logoType: string, file: File) => {
	let error = null;
	...
	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/logo/upload`, {
		method: 'POST',
		headers: {
			Authorization: `Bearer ${token}`
		},
...
export const getLogoStatus = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/logo/status`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const resetLogo = async (token: string, logoType: string) => {
	let error = null;

	const formData = new FormData();
	formData.append('logo_type', logoType);

	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/logo/reset`, {
		method: 'DELETE',
		headers: {
			Authorization: `Bearer ${token}`
		},
